...
sys.path.append("/content/haqflow-workflow")
...
if "tasks" not in st.session_state:
    ...
    st.session_state.tasks = generate_action_plan(
        DEMO_MATCH_RESULT
    )
...
for task in tasks:
    ...
    with col1:
        ...
        st.caption(
            f"Program: {task.related_program_name}"
        )
...
with st.expander("Recommended Next Actions"):
    ...
    st.write(
        handoff["recommended_next_actions"]
    )
